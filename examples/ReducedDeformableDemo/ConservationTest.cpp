...
public:
    ...
    void checkMomentum(btReducedDeformableBody* rsb)
    {
      ...
      btVector3 total_angular(0, 0, 0);
      {
        ...
        myfile << (float)sim_time << "\t" << (float)x_com[0] << "\t" << (float)x_com[1] << "\t" << (float)x_com[2] << "\n";
        myfile.close();
      }
      {
        ...
        myfile << (float)sim_time << "\t" << (float)total_linear[0] << "\t" << (float)total_linear[1] << "\t" << (float)total_linear[2] << "\n";
        myfile.close();
      }
      {
        ...
        myfile << (float)sim_time << "\t" << (float)total_angular[0] << "\t" << (float)total_angular[1] << "\t" << (float)total_angular[2] << "\n";
        myfile.close();
      }
      {
        ...
        myfile << (float)sim_time << "\t" << (float)angular_rigid[0] << "\t" << (float)angular_rigid[1] << "\t" << (float)angular_rigid[2] << "\n";
        myfile.close();
      }

      {
        std::ofstream myfile("reduced_velocity.txt", std::ios_base::app);
        myfile << (float)sim_time << "\t" << (float)rsb->m_reducedVelocity[0] << "\t" << (float)rsb->m_reducedDofs[0] << "\n";
        myfile.close();
      }
    }
