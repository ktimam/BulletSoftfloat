	...
	int solve(MatrixX& A, TVStack& x, const TVStack& b, bool verbose = false)
	{
		...
		if (r_dot_z <= Base::m_tolerance * d0)
		{
			if (verbose)
			{
				...
				std::cout << "Two norm of the residual = " << (float)r_dot_z << std::endl;
			}
			return 0;
		}
		p = z;
		...
		for (int k = 1; k <= Base::m_maxIterations; k++)
		{
			...
			if (r_dot_z_new < Base::m_tolerance * d0)
			{
				if (verbose)
				{
					std::cout << "ConjugateGradient iterations " << k << " residual = " << (float)r_dot_z_new << std::endl;
				}
				return k;
			}
		...
		if (verbose)
		{
			std::cout << "ConjugateGradient max iterations reached " << Base::m_maxIterations << " error = " << (float)r_dot_z_new << std::endl;
		}
