...
public:
	...
	virtual void testDerivative()
	{
		...
		for (int it = 0; it < 10; ++it)
		{
			...
			std::cout << "Iteration = " << it << ", f1 = " << (float)f1 << ", f2 = " << (float)f2 << ", error = " << (float)error << std::endl;
		}
		for (int i = 1; i < errors.size(); ++i)
		{
			std::cout << "Iteration = " << i << ", ratio = " << (float)(errors[i - 1] / errors[i]) << std::endl;
		}
	}
	...
	virtual void testHessian()
	{
		...
		for (int it = 0; it < 10; ++it)
		{
			...
			error = btSqrt(error);
			errors.push_back(error);
			std::cout << "Iteration = " << it << ", error = " << (float)error << std::endl;
		}
		for (int i = 1; i < errors.size(); ++i)
		{
			std::cout << "Iteration = " << i << ", ratio = " << (float)(errors[i - 1] / errors[i]) << std::endl;
