	...
	btConstraintInfo2* info, int row, btVector3& ax1, int rotational, int rotAllowed)
{
	...
	if (limot->m_enableSpring)
	{
		...
		if (rotational)
		{
			...
			if (m_rbA.getInvMass() > 0) mA = mA * rrA + 1 / (m_rbA.getInvInertiaTensorWorld() * ax1).length();
			if (m_rbB.getInvMass() > 0) mB = mB * rrB + 1 / (m_rbB.getInvInertiaTensorWorld() * ax1).length();
		}
		btScalar m;
