...
template<class FloatType> void doMathTest(string filename) {
	ofstream myfile;
  	myfile.open (filename);

	FloatType num1 = 99.8734939843940394;
	FloatType num2 = 3.29094478874893049;

	for(int i = 0; i < 100; i++)
	{
		num1 = sin(num1);
		myfile << "num1 sin : " << (float)num1 << endl;
		num1 = num1/num2;
		myfile << "num1 div : " << (float)num1 << endl;
  		num2 = cos(FloatType(-22380000000000));//cos(num1);
		myfile << "num2 cos : " << (float)num2 << endl;
		// num1 *= num2;	
		// num2 *= num1;
	
		//FLOAT_TYPE vector_length = f32_sqrt(num1 * num1 + num2 * num2);
        FloatType vector_length_sqr = num1 * num1 + num2 * num2;
		FloatType vector_length = sqrt(FloatType(vector_length_sqr));

		if (vector_length > 0)
		{
			myfile << "vector_length : " << (float)vector_length << endl;
			num1 /= vector_length;
			num2 /= vector_length;
		}

		double result = (double)num1;
		myfile << "num1 : " << result << endl;
		result = (double)num2;
		myfile << "num2 : " << result << endl;
	}
	myfile.close();
...

{
    ...
    //doMathTest<float>("test_float_linux.txt");
    doMathTest<Double>("test_soft_win.txt");
